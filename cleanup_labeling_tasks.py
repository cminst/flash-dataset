...
class LabelingTaskCleaner:
    ...
    def extract_video_info(self, video_path: str) -> Tuple[str, str, float, float]:
        ...
        # Pattern: downloaded_clips/{video_id}_{row_id}.mp4
        filename = Path(video_path).name
        if filename.endswith('.mp4'):
            filename = filename[:-4]
        ...
        parts = filename.split('_')
        if len(parts) >= 2:
            video_id = '_'.join(parts[:-1])  # Handle video IDs with underscores
            row_id = parts[-1]
        else:
            video_id = filename
            row_id = "0"
        ...
        if key in self.timing_data:
            ...
            # Fallback: assume 7-second padding around action
            original_start = 0.0
            original_end = 14.0
    ...
    def calculate_true_time(self, peak: Peak, original_start: float, original_end: float) -> Peak:
        ...
        # So the clip duration is (original_end + 7) - (original_start - 7) = original_end - original_start + 14
