...
TASK_TIMEOUT_MINUTES = 15 # If a task is 'assigned' for longer than this, it's considered stale

# A simple lock to prevent race conditions when multiple users request a video
lock = threading.Lock()

app = FastAPI()

# Pydantic model for receiving labels from the client
...
#     """
#     Serves the video files from the 'video' directory.
#     The `filename` parameter captures everything after `/video/`.
#     """
#     # Security: Basic check to prevent path traversal.
#     # This prevents requests like /video/../some_other_file.txt
#     if ".." in filename:
...
#     # Check if the file actually exists before trying to serve it
...
async def get_video(filename: str):
    ...
    # Set a proper content-type so the browser knows it’s video
    return FileResponse(file_path, media_type="video/mp4")

# --- API Routes for Labeling Logic ---

@app.get("/get-next-task")
def get_next_task():
    """
    Assigns a video to a user.
    Prioritizes stale tasks (ones that were assigned but never completed).
    This handles cases where a user refreshes or closes the page.
    """
    with lock:
        ...
        # 1. Look for stale tasks first
        stale_tasks = df[(df['status'] == 'assigned') & (df['assigned_at'] < timeout_threshold)]
        ...
        if not stale_tasks.empty:
            ...
            # 2. If no stale tasks, get a new unassigned one
            unassigned_tasks = df[df['status'] == 'unassigned']
            if unassigned_tasks.empty:
                raise HTTPException(status_code=404, detail="All tasks are completed or assigned!")
...
def submit_labels(label: Label):
    ...
    with lock:
        ...
        if not mask.any():
            raise HTTPException(status_code=404, detail=f"Row ID {label.row_id} not found.")
        ...
        df.loc[idx, 'assigned_at'] = None # Clear the assignment time
...
def get_progress():
    ...
    df = pd.read_csv(DB_FILE)
    # Total tasks are all rows that aren't marked as bad
